...
def test_weather_code_accepts_precip_probability_percent_or_fraction():
    assert weather_code_for_values(temperature_c=5, precip_probability=80).weather_code == "heavy_rain"
    assert weather_code_for_values(temperature_c=5, precip_probability=0.8).weather_code == "heavy_rain"
    assert weather_code_for_values(temperature_c=-1, precip_probability=60).weather_code == "snow"
    assert weather_code_for_values(temperature_c=-1, precip_probability=0.6).weather_code == "snow"
